...
import { firstDayOf, type TargetMonth } from '../domain/month.js';
...
export function createSubmissionsRepository(db: Database) {
	return {
		...
		// ホームの月度の状態（11-7）と月度切替の削除（11-8）。提出済みの月度を月初の暦日で返す
		async submittedMonths(months: TargetMonth[]): Promise<Map<TargetMonth, Date>> {
			const result = new Map<TargetMonth, Date>();
			...
			for (const row of rows) {
				const month = row.targetMonth.slice(0, 7) as TargetMonth;
