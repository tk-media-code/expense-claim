...
| リフレッシュトークンの置き場 | 手元の `token.json` | **サーバー側のどこか。ここが論点になる** |
...
- **どこで動かすか。** 自前のサーバーか、クラウドのサービスか。ここが決まらないと
  リダイレクトURLもトークンの置き場も決められない
- **リフレッシュトークンをどう保管するか。** 平文で置くのか、暗号化するのか
- **スコープを狭められるか。** ウェブアプリなら Google Picker を使って `drive.file` で
  済ませられる可能性があるが、検証していない

**これらは要件定義（`docs/requirements.md`）で扱う。**
