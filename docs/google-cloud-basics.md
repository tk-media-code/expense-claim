...
| `.../auth/drive.file` | アプリが作った／開いたファイルの読み書き | 領収書を既存フォルダへ保存するため。**2026-09-09 に足りると分かった** |
| `.../auth/gmail.readonly` | メールの読み取り | 依頼メールを読むため |
| `.../auth/gmail.send` | **メールの送信だけ** | 月初の提出アラートを自分あてに送るため（要件定義 7.5 / F-35） |

### `drive.file` で足りる

Drive には狭いスコープ `drive.file` がある。**アプリが作った／開いたファイルしか触れない。**
文献は、アプリが作っていない親フォルダの下にはファイルを作れないとしていた。

**2026-09-09 に `tools/drive-probe/` で実測した。** 人が手で作った既存フォルダを
`parents` に指定して `files.create` できた。共有（`anyone` / `reader`）と `webViewLink` も取れた。
親フォルダのメタデータは 404 で読めない。**アプリはフォルダIDを設定で持つので、読む必要は無い。**

**`tools/sheet-probe/` は今も広い `drive` を使う。** 委託元のシートを自分のドライブへ複製するためで、
複製元は自分が作ったファイルではない。**プローブの都合であり、アプリのスコープではない。**
...
`.../auth/gmail.send` の4つ。
**ここは検証用プローブのクライアントである。** `sheet-probe` が委託元シートを複製するため、
広い `drive` が要る。**アプリ本体のスコープは `drive.file` である**（8節）。
...
### ⑦ アプリ本体の OAuth クライアント（ウェブ アプリケーション型）— 2026-09-18 に実施

⑤ のデスクトップ型は検証ツール（`tools/*-probe`）用で、アプリ本体はスマートフォンのブラウザから
使うので **「ウェブ アプリケーション」型を別に作る**（7節 / 15節 / 設計 7.1）。同じプロジェクトの下に
クライアントは複数持てる。

1. https://console.cloud.google.com/auth/clients → 「クライアントを作成」→ 種類は **ウェブ アプリケーション**
2. **承認済みのリダイレクト URI** に、アプリの2本を登録する（設計 `05-integration.md` 3.6）。
   ローカルで確かめるときは `http://localhost:8080/api/auth/callback` と
   `http://localhost:8080/api/google/authorization/callback`。`localhost` は `http` のまま登録できた。
   本番はそのホストの `https://…` を**追加で**登録する（差し替えない）
3. 表示されるクライアント ID とシークレットをアプリの環境変数に入れる（`.env.example`）。JSON は要らない
4. 「承認済みの JavaScript 生成元」は空でよい。コードの交換はバックエンドが行い、ブラウザは Google を直接叩かない

**実測で分かったこと（2026-09-18）**

| 何が | 結果 |
| --- | --- |
| スコープの登録（④） | **要らなかった。** ④ には `drive.file` を登録していなかったが、アプリの4スコープの同意はそのまま通った。対象ユーザーが「内部」だと、登録の有無で止まらない |
| 同意画面に出るアプリ名 | **プロジェクトに1つ**で、クライアントごとではない。③ で付けた名前が、デスクトップ型でもウェブ型でも出る。変えるなら https://console.cloud.google.com/auth/branding |
| 組織外の Google アカウントでログイン | **Google 側で「組織内でのみ利用可能」として止まる。** アプリの許可アドレスの照合（設計 3.5）まで届かない。照合が効くのは組織内の別アカウントだけ |
| リフレッシュトークン | `access_type=offline` + `prompt=consent` で初回から降りてきた。DB に暗号化して保存され、以後の Sheets / Gmail / Drive の呼び出しが通った |
...
| 「組織内でのみ利用可能です」 | 組織外のアカウントで開いている。③ が「内部」なので正しい動きである（⑦） |
| `redirect_uri_mismatch` | ⑦ の 2 で登録した URI と、アプリの `GOOGLE_REDIRECT_URI_*` が1文字でも違う。末尾のスラッシュと `http` / `https` を見る |
...
**「スコープを狭められるか」は、2026-09-09 に閉じた。**

文献は「`drive.file` では、アプリが作っていない既存フォルダの下にファイルを作れない」としていた。
Picker でフォルダを選ばせる道も、フォルダは `drive.file` の意味での「ファイル」として
一覧できないため難しい、と書いてあった。

**作るほうは文献と違った。** 人が手で作った既存フォルダを `parents` に指定して、
`drive.file` だけで `files.create` できた。共有と `webViewLink` も取れた。
**読むほうは文献どおりである。** 親フォルダの `files.get` は 404 になる。
アプリはフォルダIDを設定で持つので、Picker も親の読み取りも要らない。

**8節のスコープ表は、この実測に合わせて `drive.file` へ書き換えた。**

**「ウェブアプリ型で変わること」は、2026-09-18 に実機で閉じた。** 作り方と、そのとき分かったことは 13章 ⑦ にある。
ローカル（`http://localhost:8080`）でログイン・4スコープの認可・Gmail の取り込み・ドライブへの保存・
提出シートの読み書き・提出アラートの送信まで通った。
