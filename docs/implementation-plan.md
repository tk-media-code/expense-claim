...
| --- | --- | --- | --- |
| [0](#phase-0--共通の足場) | 共通の足場 | エラーの形と API クライアントが揃う | `██████████` 3/3 |
| [1](#phase-1--駅と区間) | 駅と区間 | 駅と区間を登録し、片道運賃を持てる | `██████████` 8/8 |
| [2](#phase-2--会場とルート) | 会場とルート | 区間を並べてルートにし、会場へ紐付けられる | `██████████` 7/7 |
| [3](#phase-3--案件とホーム) | 案件とホーム | 案件を手で足し、月度ごとの一覧が見られる | `██████████` 8/8 |
| [4](#phase-4--交通費の記録) | 交通費の記録 | **「帰り道の2手」が通る** | `██████████` 6/6 |
| [5](#phase-5--認証) | 認証 | 本人だけが使える | `██████████` 6/6 |
| [6](#phase-6--google-api-の認可) | Google API の認可 | Gmail / ドライブ / シートを叩ける | `██████████` 5/5 |
| [7](#phase-7--要確認事項) | 要確認事項 | 失敗を記録して見せられる | `██████████` 3/3 |
| [8](#phase-8--スプレッドシートを読む) | スプレッドシートを読む | 会場マスタと対象月度が入る | `██████████` 4/4 |
| [9](#phase-9--gmail-の取り込み) | Gmail の取り込み | 案件が自動で入ってくる | `██████████` 5/5 |
| [10](#phase-10--タクシーと領収書) | タクシーと領収書 | 領収書がドライブへ入る | `██████████` 5/5 |
| [11](#phase-11--提出) | 提出 | **提出シートへ書き込める** | `██████████` 9/9 |
| [12](#phase-12--提出アラートと設定の控え) | 提出アラートと設定の控え | 忘れても知らせが来る | `████████░░` 4/5 |
| | **合計** | | **73/74** |
...
| # | タスク | 層 | 要件 | Issue | 状態 |
| --- | --- | --- | --- | --- | --- |
| 0-1 | API の共通エラー形式と `app.onError` を作る | BE | [`04-api.md`](design/04-api.md) 2.5 | #91 | ✅ |
| 0-2 | フロントの API クライアントと、失敗をトーストで出す共通処理を作る | FE | [`06-error-handling.md`](design/06-error-handling.md) 5章 | #93 | ✅ |
| 0-3 | 画面のルーティングとナビゲーションの骨格を作る | FE | [`02-screens.md`](design/02-screens.md) 2.1 | #95 | ✅ |

### Phase 1 — 駅と区間

| # | タスク | 層 | 要件 | Issue | 状態 |
| --- | --- | --- | --- | --- | --- |
| 1-1 | `stations` テーブルを作る | DB | F-15 | #98 | ✅ |
| 1-2 | `segments` テーブルを作る | DB | F-37 | #100 | ✅ |
| 1-3 | 駅の一覧・登録 API | BE | F-15 | #102 | ✅ |
| 1-4 | 駅の編集・削除 API（**使用中の削除は 409**） | BE | F-15 / 決定22 | #104 | ✅ |
| 1-5 | 「区間と運賃」画面の `駅` タブ | FE | [`02-screens.md`](design/02-screens.md) 3.8 | #106 | ✅ |
| 1-6 | 区間の一覧・登録 API（使用ルート数を含める。**`route_segments` が要るので 2-1・2-2 が先**） | BE | F-37 | #112 | ✅ |
...
| # | タスク | 層 | 要件 | Issue | 状態 |
| --- | --- | --- | --- | --- | --- |
| 2-1 | `venues` テーブルを作る | DB | F-13 / F-14 | #108 | ✅ |
| 2-2 | `routes` と `route_segments` テーブルを作る | DB | F-16 / F-17 | #110 | ✅ |
