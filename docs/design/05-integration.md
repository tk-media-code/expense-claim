...
決めたものである。設計フェーズの5本目で、[`04-api.md`](04-api.md) の次に置く。

- **扱うこと** — OAuth フロー、叩く呼び出しとその根拠、失敗の分け方、環境依存値
- **扱わないこと** — 異常系の実現方法（[`06-error-handling.md`](06-error-handling.md)）、
...
- **出どころ** — **要求分析 5章・6章の API 実測**、[`tools/sheet-probe/`](../../tools/sheet-probe/)・
  ...
  `01-architecture.md` 5.3 / 6.3 / 7章、`04-api.md` 2.5 / 7章
...
**`01-architecture.md` 5.3 のとおり、インターフェースと実装を分ける。**
...
**「書き込み先を実行時に決めない」（`NF-08`）を、インターフェースの形でも守る**（8.4 / `04-api.md` 7章）。

**ここを漏らすと、目的②で `fetch` へ差し替えるとき業務ロジックまで触ることになる**
（`01-architecture.md` 6.3）。
...
**`01-architecture.md` 6.3 は10本としていたが、`values.clear` が要らなくなった**（8.2）。
...
**`services` はトークンの存在を知らない**（`NF-05` / `01-architecture.md` 5.2）。

**`scheduler` も同じ行を読む。** `backend` と同じイメージを別コマンドで起動する
（`01-architecture.md` 3.8）ので、**DB を共有し、認可も共有される。認可の経路を2つ持たない。**

### 2.4 失敗をどう分けるか

**`04-api.md` 2.5 の応答へ写す。**
...
**`01-architecture.md` 3.7 が決めている。** ログインは「本人か」を確かめるだけで、
...
**混ぜると、ログインのたびに広いスコープへの同意を求めることになる**（`01-architecture.md` 3.7）。
...
ウェブアプリ型はクライアントシークレットを本当に秘密にできる（`01-architecture.md` 7.1）ので、
**認可コード横取りの前提は成立しにくい。**

**それでも使うのは、現行の指針がシークレットを持つクライアントにも求めており、
実装が数行で済むからである。** `04-api.md` 7章と同じ発想で、**塞げるものは塞ぐ。**

**`SameSite=Lax` の Cookie はトップレベルの `GET` には付く**ので、
**コールバックだけは `state` の素の照合が要る**（`04-api.md` 2.2）。
...
**どれか1つでも欠けたら 403**（F-01 / N-03 / `04-api.md` 2.5）。

**`hd`（ホストドメイン）だけで判定しない。** 組織の誰でも入れることになる。
**利用者は本人1人である**（N-03）。

**組織外の Google アカウントは、ここへ来る前に Google が止める**（2026-09-18 実測。同意画面が「内部」なので
「組織内でのみ利用可能」と出る）。手順4の照合が実際に効くのは、組織内の別アカウントに対してである。

> **①②は 2026-09-18 にローカル（`http://localhost:8080`）で実機を通した。** ウェブ アプリケーション型の
> クライアントの作り方と、そのとき分かったことは [`google-cloud-basics.md`](../google-cloud-basics.md) 13章 ⑦。

### 3.6 リダイレクト URL

**事前に登録した固定 URL**（`01-architecture.md` 7.1）。**2本を登録する。**
...
移行中も両方が動く・`01-architecture.md` 7.1）。

## 4. Gmail — 依頼メールの取り込み

**要件定義 7.2 の手順を、呼び出しへ落とす。** `POST /api/sync` の中で走る（`04-api.md` 4.3）。
...
| 使うもの | どこから |
| --- | --- |
| `id` / `threadId` / `internalDate` | メッセージの直下。**`imported_mails` に入る**（`03-database.md` 5.3） |
...
**実測で何度も直して辿り着いたものなので、書き直さない**（`01-architecture.md` 3.2）。

**置き場は `domain/` である。**

| なぜ | |
| --- | --- |
| **I/O を持たない** | 入力は文字列、出力は案件か失敗。`01-architecture.md` 5.2「`domain` はどの層にも依存しない」に素直に収まる |
...
- **`integrations/gmail/` に置く** — 委託元の書式は外部システムの都合なので一緒に閉じたくなるが、
  **`integrations` を差し替えるとき（`01-architecture.md` 6.3）に、書き直さなくてよいものまで巻き込む**
...
| どちらでもない | — | **取り込まない。** `unrelated` として記録し、**要確認事項にはしない**（決定23） |

> **件名は文書に書かれた書式と厳密に一致した**（要求分析 6章・実測）。どちらも正規表現で判別できる。
> **依頼無しメールの件名には年が入らない**が、**取り込まない種別なので支障はない。**

**同じ差出人から、依頼以外のメールも届く**（2026-09-18 実測。「8月度 給与・支払明細書のご送付」）。
`from:` で絞る以上これは混ざる。**依頼メールではないので、裏取りの失敗（F-07）とは別に扱う。**
件名の書式が変わったときは、月曜のあとに案件が入っていないことで気づく（決定23）。
...
| 落とすもの | なぜ |
| --- | --- |
| **リンク**（`extractLinks`） | **叩かない**（要件定義 7.2）。委託元の配信サービスの計測用 URL で、**叩けば計測される**。案件承諾は対象外（要件定義 1章） |
| **スタッフの割当**（`extractAssignments`） | 提出に要らない。**他人の氏名を持たない**（N-18） |
| 挙式・披露宴の時刻／緊急連絡先 | 提出に要らない |

**取り出すのは5項目だけである** — 施行日・会場名・会場コード・ご両家名・案件番号（F-06）。
**目的（C列）は常に `婚礼案件` なので、メールから取らない**（要件定義 5.3 / `03-database.md` 5.2）。
...
| 件名がどちらでもない（依頼以外のメール） | `unrelated` | 作らない。**要確認事項にはしない**（決定23） |
| 件名は案件詳細だが、**裏取りが失敗** | `parse_failed` | 作らない。**要確認事項に残す**（F-07） |

**どの結果でも行を残す**（要件定義 6.3 / `03-database.md` 5.3）。
...
| | |
| --- | --- |
| **誰が呼ぶか** | **`scheduler` コンテナだけ**（`01-architecture.md` 3.8）。**API から送る経路を持たない**（`04-api.md` 7章） |
...
**要件定義 7.3。この順で通す**（`04-api.md` 4.6 の `POST /api/projects/:id/taxi-rides` 1本の中）。
...
| **ファイル名はアプリが付ける** | `<YYYYMMDD>_<会場コード>_<連番>`（`03-database.md` 5.2）。**クライアントから受け取らない**（`04-api.md` 7章） |
| **`fields` に `id,webViewLink` を指定する** | **既定では返らない。** `webViewLink` が `receipts.drive_url` になる |
| **URL を保存する。組み立て直さない** | `03-database.md` 5.2。**ドライブの URL 書式が変わったとき、過去に提出した URL と食い違う** |
...
**`files.delete` を呼ぶ経路を持たない**（`04-api.md` 7章）。
...
`services` が受け取るのは**対象月度の型**である（`01-architecture.md` 5.4）。

**日付として解釈できなければ、提出を中止する**（要件定義 5.5 / `04-api.md` 5.4）。
**対象月度が決まらないまま書けば、他人ではなく自分の申請を壊す。**

**この値が月度切替の検知にも使われる**（F-32 / `03-database.md` 5.3 の `last_seen_target_month`）。

### 7.2 構造 — 行数と保護範囲を実行時に読む
...
**変わったら止まって知らせるのが要件であって**（`NF-10` / 要件定義 1章）、
...
**行の作り方と列の規則は要件定義 5.3、値の出どころは `03-database.md` 7.2 にある。**

**`integrations` は組み立てない。** 受け取った行をそのまま書く（2.1）。
**「加工は記録時に済ませ、提出時には何もしない」**（`03-database.md` 5.2）が、ここまで効いている。

### 8.2 1回の `values.update` に畳む
...
| なぜこうするか | |
| --- | --- |
| **2操作の原子性に寄りかからない** | **矩形1つなら、「クリアだけ通った」状態が作れない** |
...
**すべて環境変数**（`NF-07` / N-08 / N-18 / `01-architecture.md` 6.1 の縛り3）。
...
| `SESSION_SECRET` | セッション Cookie の署名鍵 | `04-api.md` 4.1 |
...
| `NF-11` | **外部連携が関わらない。** 設定データの控えは DB だけで完結する（`01-architecture.md` 8章） |
...
| --- | --- | --- |
| **`googleapis` が Workers で動くか** | Node.js 依存が Workers ランタイムで動くか | **目的②のプロジェクト**（`01-architecture.md` 10章） |
| **`text/html` からテキストを起こす方法** | `tools/gmail-probe/` はタグを落とす自前の実装で足りた。**ライブラリを使うかは決めていない**（Node.js 依存の強いものは目的②で外すことになる。`01-architecture.md` 2.3） | 実装時 |
| 領収書のサイズ上限 | `04-api.md` 10章から引き継ぐ | 実装時 |

閉じた宿題（`drive.file`、トランザクション境界、`detail` の書式、cron の死活）は
[`docs/decisions.md`](../decisions.md) と [`06-error-handling.md`](06-error-handling.md) を見る。
`drive.file` で済むことは 3.3 にある。

③ `permissions.create` が失敗すると、DB には何も残らないが、②で作ったファイルは残る
（`06-error-handling.md` 6.2）。孤児のファイルができるが、これを許す。
消す実装を足せば「消してよいものを消す」判断がアプリに増え、
しかも消す呼び出し自体が失敗しうるので、孤児が出る経路は結局消えない。
提出シートに載るのは `receipts.drive_url` だけであり、④を通ったものしか載らない。

> **「セル上の画像」を挿入するリクエストは Sheets API に無い**（要求分析 5.2）。
> 委託元の「入力例」シートは画像の直接添付を想定しているが、
> **アプリが採れるのは URL 方式だけである**（要件定義 1章）。
