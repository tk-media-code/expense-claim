...
| --- | --- | --- |
| **`googleapis` が Workers で動くか** | バンドルが 3 MiB に収まるか | **目的②のプロジェクト**（`01-architecture.md` 10章） |
| **`text/html` からテキストを起こす方法** | `tools/gmail-probe/` はタグを落とす自前の実装で足りた。**ライブラリを使うかは決めていない**（`01-architecture.md` 6.1 の縛り1・バンドルに効く） | 実装時 |
