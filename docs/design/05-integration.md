...
| 宿題 | どこから来たか | どこで閉じるか |
| --- | --- | --- |
| **`NF-09` 提出シートの行数・保護範囲を決め打ちにしない** | `01-architecture.md` 9章 | **7.2** |
| **`multipart` の受け方**（Hono → `googleapis`） | `04-api.md` 10章 | **6.2** |
| **一括更新の原子性** | 要件定義 13章 / `01-architecture.md` 10章 | **8.2。確かめる必要が無くなった** |
...
### 6.2 `multipart` の受け方 — `04-api.md` 10章の宿題
...
**引き継いでいた3件は、[`06-error-handling.md`](06-error-handling.md) で閉じた。**

| 何が | どう決まったか |
| --- | --- |
| **トランザクションの境界** | **外部呼び出しをまたいで開かない**（`06-error-handling.md` 6.1）。6.1 の①〜③はトランザクションの外に置き、**囲むのは④の `taxi_rides` と `receipts` だけ**（同 6.2） |
| **要確認事項の `detail` の書式** | **素のテキスト1本**（同 4.1）。`sheet_unreachable` の文面には、**2.4 が分けた「認可切れか共有停止か」を入れる。** 導く先が違うので、**文面で分かれていなければ意味が無い**（同 4.2） |
| **定期実行が落ちたことに気づく手立て** | **cron は起動するたびに時刻を残し、それをホームに出す**（同 7.2）。**アラートを送る仕組みが落ちたことを、アラートで知らせることはできない** |

**6.3「消さない」が、そのまま結論に効いた。**
