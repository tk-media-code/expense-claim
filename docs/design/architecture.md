...
| **常時稼働の定期実行を持たない** | NF-04 / 決定1 | cron・ジョブキュー・ワーカーを置かない |
...
| セッション | **署名付き Cookie**（`HttpOnly` / `Secure` / `SameSite=Lax`） |
...
**EC2 が常時起動であることは、`NF-04`（常時稼働の定期実行を持たない）と矛盾しない。**
`NF-04` が禁じているのは**定期実行そのもの**であって、サーバーが起きていることではない。
**cron を置かなければ守れる。** 取り込みも月度切替の検知も、リクエストの中で走る（決定1）。
...
| 相手 | 使う呼び出し |
| --- | --- |
| Gmail | `messages.list` / `messages.get` |
...
| `NF-03` 3層 | 4.1 / 4.2 |
| `NF-04` 常時稼働の定期実行なし | 3.8（cron を置かない） |
...
| 何が | どこで決めるか |
| --- | --- |
| 領収書のファイル名の命名規則 | [`integration.md`](integration.md)（要件定義 13章） |
| テーブル定義・型・索引 | [`database.md`](database.md) |
| エンドポイントの粒度 | [`api.md`](api.md) |
| セッションの有効期限 | [`api.md`](api.md) |
