...
**置き場（EC2 の cron・systemd タイマー・専用コンテナのいずれか）は、まだ決めていない**（11章）。
`docker compose` の中に置くか外に置くかで、EC2 のホスト側に手が入るかどうかが変わる。
...
| **提出アラートの定期実行の置き場** | 3.8。EC2 の cron・systemd タイマー・専用コンテナのいずれか。**実行が落ちたときに気づく手立ても含めて決める**（要件定義 13章） |
