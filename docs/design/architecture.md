...
| `NF-10` 様式が変わったら止まる | [`error-handling.md`](error-handling.md) 5.1（**中止条件が、すべて書き込みより前に並んでいる**） |
| `NF-11` 設定データの控え | 8章 |
...
| 何が | どう決まったか |
| --- | --- |
| エンドポイントの粒度 | **リソース中心に、集約を2本だけ混ぜる**（`api.md` 3章）。集約するのはホームと交通費の記録で、**どちらも「2手」の経路上にあり、開いた時点で揃っていることが要件そのもの**だから |
...
| 何が | どう決まったか |
| --- | --- |
| **定期実行が落ちたことに気づく手立て** | **cron は起動するたびに時刻を残し、それをホームに出す**（`error-handling.md` 7.2 / [`database.md`](database.md) 5.3 の `sync_state.last_cron_run_at`）。**要確認事項には積まない** — 落ちている cron に自分の不在は記録できず、**「無いこと」は記録できない。** あわせてコンテナに `restart: unless-stopped` を入れる（**回復であって検知ではない。両方が要る**） |
| **`NF-10` 様式が変わったら止まる** | **要件定義 5.2 の手順の並びが、そのまま実装になっている**（`error-handling.md` 5.1）。中止条件は5つとも書き込みより前にあり、**7行目のヘッダーを読むのは手順3である** |
