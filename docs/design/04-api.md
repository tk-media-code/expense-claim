...
| 2 | 変わっていれば、**切替先より前で、かつ提出が済んだ月度の実績を消す** | F-32 / `03-database.md` 6.3 |
| 3 | 消さずに残ったもののうち、**切替先より前で未提出のもの**を要確認事項に残す | F-32 / `02-screens.md` 4.4 |
| 4 | 依頼メールを取り込む | F-03〜F-08 |
| 5 | `last_imported_at` を更新する。**1件も取り込まなかった実行でも更新する** | `03-database.md` 5.3 |
...
**重複の判定からは自分自身を除く。** **重複は向きを問わない**（決定24）。逆向きが登録済みのときは
409 の `message` で「逆向きで既に登録されています」と言う。復路は同じ区間を逆向きに使うので、登録は要らない。
