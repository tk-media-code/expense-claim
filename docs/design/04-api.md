...
| 宿題 | どこから来たか | どこで閉じるか |
| --- | --- | --- |
| **エンドポイントの粒度** | `01-architecture.md` 11章 | **3章** |
| **全端末のセッション失効** | `01-architecture.md` 11章 / `03-database.md` 12章。**署名付き Cookie でサーバー側に状態が無い** | **4.1 / 7章** |
...
**`01-architecture.md` 11章がここへ持ち越した宿題を閉じる。**
...
#### 全端末のセッション失効 — `01-architecture.md` 11章 / `03-database.md` 12章の宿題
...
濁点だけが違う駅名も別の行になる（`03-database.md` 4.1・**実機未確認**）。
...
> 実機では確かめていない**（要件定義 13章）。**`05-integration.md` と実装時に確かめる。**
...
`03-database.md` 6.2 の RESTRICT は、**使われているものを消させない**という形でそれを実現している。
...
**登録し間違えたものを消す手段が無く、実際に困った**（`02-screens.md` 3.8）。
...
**`03-database.md` 12章から引き継いだ2件は、[`06-error-handling.md`](06-error-handling.md) で閉じた。**
**どちらも「失敗したときにどう振る舞うか」であり、あの文書の主題そのものだった。**

| 何が | どう決まったか |
| --- | --- |
| **`attentions` の `detail` の書式** | **素のテキスト1本**（`06-error-handling.md` 4.1）。2.5 の `message` と同じ「本人が読む日本語」にそろえた。**画面から見て、要確認事項とエラーが同じ扱いになる** |
| **トランザクションの境界** | **外部呼び出しをまたいで開かない**（`06-error-handling.md` 6.1）。提出は**シートが先・`submissions` が後**で、間で落ちたら **`200` を返さない**（同 6.3）。シートは書けているので、**もう一度実行すれば揃う**（`F-29`） |

**`GET /api/home` の応答に `lastCronRunAt` が1つ増えている**（4.3）。
`06-error-handling.md` 7章が、**cron が落ちたことに気づく手立て**を
「走った時刻を残し、ホームに出す」と決めた結果である。
**要確認事項には積まない** — 積む主体がいないからで、**「無いこと」は記録できない。**

**「`multipart` の受け方」は、[`05-integration.md`](05-integration.md) 6.2 で閉じた。**
