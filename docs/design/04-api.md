...
設計フェーズの4本目で、[`01-architecture.md`](01-architecture.md)・[`02-screens.md`](02-screens.md)・
[`03-database.md`](03-database.md) の次に置く。

- **扱うこと** — エンドポイントの粒度と一覧、要求と応答の形、エラーの返し方、
  **持たないエンドポイント**、画面との対応
- **扱わないこと** — 外部 API の呼び出し設計（[`05-integration.md`](05-integration.md)）、
  異常系の実現方法（[`06-error-handling.md`](06-error-handling.md)）、画面の見た目（[`02-screens.md`](02-screens.md)）
- **出どころ** — 要件定義 4章（機能要件）・5章（書き出し仕様）・9章（非機能要件）、
  `01-architecture.md` 3.7 / 4.3 / 5.2、`02-screens.md` 2.x / 3.x / 4.x、`03-database.md` 5.x / 7.x
...
**`01-architecture.md` 5.4 が「取り違えられないよう型で分ける」としたのと同じ発想を、API の面でやる。**
検証で弾くのではなく、**そもそも書けないようにする。**

**本文はいまの作り方だけを書く。** 判断がひっくり返ったら本文を書き直し、経緯は
[`docs/decisions.md`](../decisions.md) へ移す。前の文書の結論が変わる場合は、そちらも直す。
...
| 前提 | 出典 | API への効き方 |
| --- | --- | --- |
| **同一オリジンでパスを振り分ける** | `01-architecture.md` 4.3 | **CORS を作らない。** ベースパスは `/api` |
| セッションは署名付き Cookie。90日・使うたび延びる | `01-architecture.md` 3.7 | トークンをヘッダーで運ばない（2.3） |
| **Google API を叩くのは `integrations` だけ。認可情報を外へ出さない** | `01-architecture.md` 5.2 / `NF-05` | **トークンを返すエンドポイントを持たない**（7章） |
| `routes` は `services` だけを呼ぶ | `01-architecture.md` 5.2 | エンドポイントは業務ロジックを持たない。**この文書は入口の形だけを決める** |
| **「月」の3基準を型で分ける** | `01-architecture.md` 5.4 | **JSON の書式で分ける**（2.4） |
| **取り込みの完了を待たずに一覧を出す** | `02-screens.md` 3.2 | **一覧と同期を別のリクエストにする**（3.1） |
| **記録画面は開いた時点で保存できる状態** | `02-screens.md` 3.5 / 4.3 | **既定値をサーバーが返す**（3.2 / 5.2） |
| **加工は記録時に済ませる。提出時には何もしない** | `03-database.md` 5.2 | **提出行の正本を作るのはサーバー**（3.2 / 5.3） |
...
**オリジンが分かれないので、CORS の設定が要らない**（`01-architecture.md` 4.3）。
...
**`01-architecture.md` 5.4 が型でやったことを、API の面でやる。**
...
`03-database.md` 4.2 が `DATETIME` を UTC で入れると決めており、**そのまま出す。**
**表示のための JST への変換はクライアントが行う。**

**`YYYY-MM-DD` に時刻もタイムゾーンも付けない。**
施行日は暦日であって時刻ではない（`03-database.md` 4.2）。
...
**`01-architecture.md` 3.7 がログインの認可と Google API の認可を分けたので、失われ方も2つある。**
...
| **非同期ジョブと、その状態を問い合わせるエンドポイント** | ジョブキューもワーカーも置かない（`NF-04` / `01-architecture.md` 2.1）。**同期は同期で返す**（3.1） |
...
**集約するのは2本だけである。** どちらも `02-screens.md` 2.2 の**2手の経路上にある画面**で、
**「開いた時点で必要なものが揃っている」ことが要件そのもの**だからである。

| 画面 | 資源単位だと | 集約すると | 根拠 |
| --- | --- | --- | --- |
| **ホーム** | 案件・対象月度と提出状態・要確認件数・最後に取り込んだ日時で**4往復** | **1往復** | `NF-12` / `02-screens.md` 3.2 |
| **交通費の記録** | 案件・会場のルート・既存の記録で**2〜3往復**。**そのあと既定値を組み立てる** | **1往復。既定値まで入って返る** | 要件定義 8.1 / `02-screens.md` 3.5 |

**逆に、会場とルートの画面は2手の経路に無い**（`02-screens.md` 2.1）。
**往復回数を減らす理由が無いので、資源単位に置く。**

```mermaid
flowchart LR
    subgraph path["2手の経路（02-screens.md 2.2）"]
        h["① ホーム"] --> r["② 交通費の記録"]
    end
...
**案件の詳細を集約しないのは、決定19 でここが経路から外れたからである**（`02-screens.md` 2.3）。
...
| 何を | なぜ |
| --- | --- |
| **案件の月度** | 決定12。`03-database.md` 9章が列すら持たないと決めた値である。**クライアントで導出させると、そこが2つ目の答えになる** |
| **対象月度** | 決定13。提出シートの `A1` が正。**手で変えられる経路を作らない**（7章） |
| **並び順** | 要件定義 5.3（施行日の昇順、同じ日なら案件番号の昇順）。**提出行と同じ規則をホームでも使う** |
| **記録画面の既定値** | 「ルートが1本ならそれ、複数なら未選択」（`02-screens.md` 4.3）。**2手が成立するかどうかがこの規則にかかっている** |
| **提出行に書く金額**（往復の ×2、復路の反転） | `03-database.md` 5.2。**`expense_record_legs` は提出行の正本である。正本を作る規則を1か所に置く**（5.3） |
...
| 何を | なぜ |
| --- | --- |
| **「施行前」の強調** | 施行日と今日を比べるだけ（`02-screens.md` 4.1）。**間違えても表示が変わるだけで、データは壊れない** |
| **合計額の表示** | 区間の金額の足し算。**提出シートへ書くのは区間ごとの額であって、合計ではない**（要件定義 5.3） |
| **要確認事項が0件のとき出さないこと** | `02-screens.md` 4.4。表示の判断 |
...
**`/api/auth/login` と `/api/auth/callback` には認証を被せない。** ログイン前に叩く入口で、
被せようがない。4.11 の「認証を要らない唯一のエンドポイント」は、業務の入口についての話である。
**許可アドレス以外の 403 は、ブラウザには JSON ではなくログイン画面の文面で見せる**
（`/login?error=…` へ 302）。コールバックはリダイレクトの途中で、JSON を出しても本人が読めない。
...
| 4 | 通ったら、**`iat` を今にして Cookie を発行し直す**（使うたび延びる・`01-architecture.md` 3.7） |
...
**`01-architecture.md` 3.7 は有効期限を90日と長く取った。** 帰り道に再ログインを求めないためであり、
...
| **`03-database.md` 5.3 の「セッションのテーブルを持たない」を覆さない** | 持つのは**端末の一覧ではなく、失効の基準時刻1つ**である |

**採らなかった案**

- **端末ごとのセッションテーブル** — 個別の端末を選んで切れる。**だが期限切れ行の掃除が要り、
  `03-database.md` 5.3 が明示的に「持たない」と決めたものを覆すことになる。**
  端末1台に対して、得るものより増える手当てのほうが多い
- **Cookie の署名鍵を入れ替える** — テーブルが1つも増えない。**だが鍵は環境変数なので、
  入れ替えるには再起動が要る**（`01-architecture.md` 7.3）。
  `01-architecture.md` 7.2 がリフレッシュトークンを環境変数に置かなかったのと**同じ理由で成立しない**
...
**`missingScopes` を返すのは、スコープが増えたときに気づくためである**（`03-database.md` 5.3）。
...
| | パス | 何をするか | 要件 |
| --- | --- | --- | --- |
| GET | `/api/home` | **集約。** 月度ごとの案件・対象月度・提出状態・未確認の要確認件数・最後に取り込んだ日時・**提出アラートが最後に動いた日時** | F-09 / F-21 / F-35 |
| POST | `/api/sync` | **依頼メールの取り込みと、月度切り替わりの検知** | F-03〜F-08 / F-32 |

**`GET /api/home` は DB しか読まない。** 外部 API を叩かない。
**`02-screens.md` 3.2 の「取り込みの完了を待たずに一覧を出す」が、この分け方そのものである。**
1本にすると、**Gmail を待ってから一覧が出る。2手の1手目が遅れる**（`NF-12`）。

#### `POST /api/sync` が2つを兼ねる理由

要件定義 2.1 (3) は「**依頼メールの取り込みも、月度切り替わりの検知も、アプリを開いたときに走る**」と、
2つを1組で書いている。`03-database.md` の `sync_state` も、
...
| 2 | 変わっていれば、**切替先より前の月度の実績を、提出の有無を問わず消す** | F-32 / 決定27 / `03-database.md` 6.3 |
| 3 | 依頼メールを取り込む | F-03〜F-08 |
| 4 | `last_imported_at` を更新する。**1件も取り込まなかった実行でも更新する** | `03-database.md` 5.3 |
...
**同じメールを2度取り込めない**（F-08 / `03-database.md` 5.3）。
...
（`03-database.md` 5.2）。**受け取ると、取り込んだ案件を手動と偽れる。**

**案件番号が既存と重複したら 409（`PROJECT_NO_DUPLICATED`）。**
`projects.UNIQUE (project_no)` が二重取り込みの一段目である（F-08）。
**案件番号が空なら 422（`INVALID_VALUE`）。** 案件番号は必須である（F-10 / `02-screens.md` 3.4）。
**必須にしたことで、手で足した案件にも二重取り込みの防止が効く**（要件定義 6.2）。
...
**`PUT` にするのは、1案件に交通費記録が0か1だからである**（`03-database.md` 5.2 の `UNIQUE (project_id)`）。
**作るのも直すのも同じ操作になる。** 記録済みの案件をもう一度開いて直しても、同じ入口で済む。

**記録を消すエンドポイントは持たない。** `02-screens.md` 3.5 に操作が無い。
...
`02-screens.md` 3.5 は、記録画面の最後に「保存」を置いている。
**タクシーもそこでまとめて送りたくなるが、採らない。**

| なぜ | |
| --- | --- |
| **DB の形がそうなっている** | `taxi_rides` は `projects` にぶら下がり、`expense_records` の子ではない（`03-database.md` 5.2）。**乗車は交通費記録と独立している** |
...
**重複の判定からは自分自身を除く。** **同じ2駅は向きが逆でも同じ区間である**（決定28）。
区間は「自宅→会場」の向きで1つだけ持ち、復路はそれを反転して使うので、逆向きの登録は要らない。
逆向きが登録済みのときは 409 の `message` でその規則を言う。「既に登録されています」だけでは、
一覧に同じ向きの区間が見当たらず戸惑う。
...
`03-database.md` 6.2 の `segments → route_segments` は RESTRICT なので、
**そもそも DB が消させない。** API はそれを 409 として言い直しているだけである。

使われているものは消せない（決定22）。経緯は決定ログを見る。
...
**同じ理由で、絞り込みの検索パラメータも持たない**（F-38 / `02-screens.md` 3.6）。
**43件を全件返しているので、絞り込みは画面が行う。**
サーバーへ寄せると、打つたびに往復が要る。

**`POST /api/venues/import` が `source = 'manual'` の行を触らない**（`03-database.md` 5.1）。
毎回入れ直すと、**手で足した会場を巻き込んで消しかねない。**

**会場を削除するエンドポイントを持たない。** `02-screens.md` 3.6 に操作が無く、
`03-database.md` 6.2 も `venues → routes` を RESTRICT にしている。**設定データを巻き込みで消さない。**
...
`to_station_name` に**記録した時点の文字列**で持ち、駅への外部キーではない（`03-database.md` 7章）。
`03-database.md` は駅名を直す操作を**はじめから想定していて**、そのためにこの列を作っている。

**駅名が既存と重複したら 409。** `stations.UNIQUE (name)` に当たる。**`POST` も `PUT` も同じ**で、
**`PUT` の判定からは自分自身を除く。**
**`X鉄乙駅` と `Y鉄乙駅` は別の駅である**（F-15 / 要求分析 5.2）。
濁点だけが違う駅名も別の行になる（`03-database.md` 4.1・実機で確認済み）。
...
**未確認へ戻す操作は持たない**（`02-screens.md` 3.10 に無い）。
...
**`GET /api/settings` が返すのは名前だけである**（N-08 / `02-screens.md` 3.11）。
...
**public リポジトリの方針とは別に、応答にも出さない**（`02-screens.md` 5章）。

**控えは駅・会場・ルート・区間だけを含む**（`01-architecture.md` 8章）。
...
**業務のためではなく、運用のために置く**（2026-09-09 / Issue #75）。
compose の `backend` の healthcheck と、**nginx から backend への振り分けが生きていることを
機械で確かめる**ために要る。

**認証を要らない唯一のエンドポイントである。** 返すのは固定の応答だけで、
データベースにも外部にも触らない。**業務データを持たないので `NF-06` に反しない。**

**2.7「持たないもの」には触れない。** あそこが挙げているのは、
ページング・購読・非同期ジョブ・バッチ・バージョニングという**業務の形**である。
...
| 決めたこと | なぜ |
| --- | --- |
| **`months` で束ねて返す** | ホームは月度ごとに区切る（`02-screens.md` 4.2）。**束ねるのは月度の判定であり、サーバーの仕事**（3.2） |
| `state` は `due` / `submitted` / `upcoming` の3つ | `02-screens.md` 3.2 の「提出待ち／提出済み（日時）／これから稼働」 |
| **並び順は月度の降順、月度の中では施行日の昇順** | `02-screens.md` 3.2。**配列の順序がそのまま画面の順序である** |
| **`attentionCount` は未確認だけを数える** | `02-screens.md` 4.4。**0件のとき出さないのはクライアントの判断**（3.2） |
...
**区間も駅名も金額も送らない。送るのは、選んだルートだけである。**

| なぜ | |
| --- | --- |
| **`expense_record_legs` は提出行の正本である**（`03-database.md` 5.2 の見出し） | **正本を作る規則を1か所に置く。** クライアントが組み立てた区間をそのまま保存すると、**ブラウザの中に提出行の作り方ができる** |
| **往復の ×2 と復路の反転は、要件が定めた規則である**（要件定義 5.3 / 決定9） | **要件の規則をクライアントへ置かない**（3.2） |
| **金額に人の判断が入らない**（F-20 / `02-screens.md` 3.5） | **記録画面に入力欄が無い。** 2手の経路では、**金額を1円も送らない。**金額を直す入口は `PUT /api/segments/:id`（4.7）だけである |
...
**負の運賃を弾くのは `POST /api/segments` と `PUT /api/segments/:id` である**（4.7）。
`one_way_fare` は `INT UNSIGNED` だが、**DB は二重の網であって一枚目ではない**
（N-09 / `03-database.md` 4.4）。**入口で弾く。**
...
**`fileName` はアプリが付ける**（`03-database.md` 5.2 の `<YYYYMMDD>_<会場コード>_<連番>`）。
...
実現の形は [`05-integration.md`](05-integration.md) 8.2 にある。
本文行の矩形を1回の書き込みで送るので、「クリアだけ通った」状態は作れない。
...
| **書き込み先を受け取るパラメータ**（スプレッドシートID・シート名・フォルダID） | **`NF-08` / N-12。** 環境変数から取る。**実行時に決めない**（`01-architecture.md` 2.1） |
| **メールを送るエンドポイント** | **`NF-13` / N-23。** 送るのは `scheduler` コンテナだけ（`01-architecture.md` 3.8）。**宛先を組み立てる経路が API に無い** |
| **対象月度を切り替えるパラメータ** | **決定13 / `02-screens.md` 5章。** `A1` が正。**手で変えられると判断が2つになる**（6.2 の `targetMonth` は照合のためだけで、書き込み先を選ばない） |
| **ID を返す応答**（スプレッドシートID・フォルダID・ドライブのファイルID・Gmail の message id） | **N-08。** 設定は名前だけ、領収書は URL だけ（4.10 / 5.5） |
| **`source` を受け取るパラメータ**（案件・会場） | 取り込みで作れば `mail` / `master`、手で作れば `manual`（`03-database.md` 5.1 / 5.2）。**偽れる経路を作らない** |
| **依頼メールの本文を返すエンドポイント** | `02-screens.md` 5章。取り出した5項目で足りる。**リンクは叩かない**（要件定義 7.2） |
...
| **会場を削除するエンドポイント** | `02-screens.md` 3.6 に操作が無い。`03-database.md` 6.2 が `venues → routes` を RESTRICT にしている。**設定データを巻き込みで消さない**（区間・駅は 4.7 へ移した。下記） |
| **提出済みデータを読むエンドポイント** | 要件定義 1章。**正本は提出シートにある**（N-05） |
| **ドライブのファイルを消すエンドポイント** | 要件定義 6.4 / 1章。**アプリが消してよいものではない** |
| **`scheduler` 専用のエンドポイント** | `scheduler` は `backend` と**同じイメージを別コマンドで起動する**（`01-architecture.md` 3.8）。**HTTP を経由しないので、外から叩ける入口が生まれない** |
| **管理者・他ユーザーのためのエンドポイント** | 利用者は本人1人（N-03）。**権限モデルを持たない**（`01-architecture.md` 3.7） |

#### 区間・駅は消せる。会場は消さない

区間と駅は、使われていないものに限って消せる（4.7 / 決定22）。
この表が守っているのは「設定データを巻き込みで消さない」である。
`03-database.md` 6.2 の RESTRICT は、使われているものを消させない。巻き込みは1件も起きない。

会場だけこの表に残す。案件は会場を会場コードの文字列で持ち、外部キーではない。
会場を消すと、ルートが0本でも、その会場コードの案件が「未登録の会場」表示に落ちる。
駅・区間にはこれが無い（実績は駅名を記録時点の文字列で持ち、区間を参照していない）。
加えて会場は `POST /api/venues/import` で入れ直せる。

**下の3行は、`01-architecture.md` 5.4 と同じことをしている。**
...
**`02-screens.md` 2.1 の11画面すべてを埋める。**
...
**ホームを開くと `GET /api/home` と `POST /api/sync` が両方走る**（F-03 / `02-screens.md` 3.2）。
**`GET` の結果を待たずに `POST` を投げ、`POST` が返ったら `GET` をやり直す。**
**一覧は先に出る。** 取り込み中はその旨を出す。

**ルートの編集で `GET /api/segments` を先に取るのは、区間を選ばせるためである**（`02-screens.md` 3.7）。
**駅を選ばせるのは「区間と運賃」の側へ移った**（`02-screens.md` 3.8）。
...
| F-33 / F-34 | 4.9 |
| **F-35 / F-36** | **エンドポイントを持たない**（7章）。`scheduler` が送る（`01-architecture.md` 3.8） |
...
| **領収書のサイズ上限** | **20MB にした**（2026-09-15）。nginx の `client_max_body_size` と揃え、超えたら 422 `INVALID_VALUE`。1件あたり数百KB〜数MB（要件定義 9.1）に対して十分な余裕がある | 閉じた |
...
閉じた宿題（`detail` の書式、トランザクション境界、`multipart` の受け方、cron の死活）は
[`docs/decisions.md`](../decisions.md) と決まった先の文書を見る。
`GET /api/home` の `lastCronRunAt` は [`06-error-handling.md`](06-error-handling.md) 7章、
`multipart` は [`05-integration.md`](05-integration.md) 6.2。
