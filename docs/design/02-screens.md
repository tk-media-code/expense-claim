...
**太い線が「帰り道の経路」である**（2.2 / 決定19）。ホームから交通費の記録へ直行する。
...
パスは [`04-api.md`](04-api.md) のリソース名（`projects` / `venues` / `routes` / `segments` / `attentions` / `settings`）に揃える。
提出だけは「確認 → 実行 → 結果」という操作の画面なので `/submit`。

戻り先は **履歴ではなく固定の親** である。直リンクや再読み込みのあとでも同じ場所へ戻る。
記録画面だけは 3.5 が「開いた画面へ帰る」と決めているので、詳細から開くときは `?from=detail` を付ける。

寄り道の登録（会場・駅・区間のシート）は画面を替えないので URL を持たない。

| 画面 | パス | 戻り先 | 中身を作る Phase |
| --- | --- | --- | --- |
| ログイン | `/login` | — | 5-6 |
| ホーム | `/` | —（**ここだけアプリバーに「案件を追加」とメニュー**） | 3-6 |
| 案件の詳細 | `/projects/:id` | ホーム | 3-8 |
| 案件の追加 | `/projects/new` | ホーム | 3-7 |
| 交通費の記録 | `/projects/:id/record` | **開いた画面。** 既定はホーム。詳細から開くときは `?from=detail` を付け、詳細へ戻る（3.5） | 4-4 |
| 会場とルート | `/venues` | ホーム | 2-6 |
| ルートの編集 | `/routes/new` ／ `/routes/:id` | 会場とルート | 2-7 |
| 区間と運賃 | `/segments` | ホーム | 1-5 / 1-8 |
| 提出 | `/submit` | ホーム | 11-6 |
| 要確認事項 | `/attentions` | ホーム | 7-3 |
| 設定 | `/settings` | ホーム | 6-5 |
...
> 押しやすさ・誤タップ・スクロール量は、2.5 の Artifact をスマートフォンで開いて確かめた（決定19）。
...
同じ日に案件が2件以上あることがある（決定4 / 要件定義 5.6）。
間違った案件に記録すると、提出まで気づけない。
確認に要る3項目は、詳細を挟まなくても見える。
...
長押しで直接記録へは採らない。ボタンを置いたほうが、押せる場所が目に見える。
経緯は決定19 を見る。
...
| 切り替わった後 | 9/6（日） | **`2026-09`** | **提出せずに切り替わり、8月度が残る**（モックは決定27 より前のもの。いまは提出していなくても消える） |
...
| 要確認事項 | **7種別すべて**（3.10。モックは決定27 より前のもので、廃止した「提出せずに月度が切り替わった」も持つ） |
...
> **モックは見た目の正本ではない**（2026-09-09 / Issue #74）。
> **正本なのは UI の機能と導線**、つまり 2.1 の遷移・3章の項目・4章の状態と分岐である。
> **配色・タイポグラフィ・ダークモードの作り込みは実装へ移植しない。**
> 実装との突き合わせは「同じ見た目か」ではなく「**同じことができるか**」で見る。
...
**「自宅→会場」の向きで登録する**（決定9 / 決定28）。見出しにその向きを添え、どちら向きに並べるかを画面で言う。
...
**区間は「自宅→会場」の向きで登録する**（決定9 / 決定28）。出発駅が自宅側、到着駅が会場側である。
ラベルに「（自宅側）」「（会場側）」を添えるのは、どちらの欄にどの駅を入れるかを画面で言うためで、
説明文（決定25）ではない。逆向きの区間は同じ区間なので登録できず、409 の文面がその規則を言う（`04-api.md` 4.7）。
...
| | **書き込み先** | 表示 | **スプレッドシートの名前と自分のシートの名前**（[`04-api.md`](04-api.md) 5.4）。ID は出さない（N-08） |
...
**書き込み先を確認の段に出すのは、環境変数の指し先を本人が実行前に確かめるためである。**
2026-09-18 に、サンドボックスのつもりで `SPREADSHEET_ID` に本番のシートが入ったまま提出し、
本番の自分のシートに書いた。名前を出していれば実行前に気づけた。設定画面（3.11）と同じく名前だけで、ID は出さない。
**実行してから知らせるのでは遅い。**
...
| 翌月4〜7日 | 月度が切り替わり、**前月分が消える。提出していなくても消える**（F-32 / 決定27） |
...
| 出さないもの | なぜ |
| --- | --- |
| 提出済みデータの閲覧・集計 | 対象外（要件定義 1章）。正本は提出シートにある |
...
> **画面の見た目は閉じた**（2026-09-09 / Issue #74）。
> **Nuxt UI 4 の既定テーマに委ねる**（[`01-architecture.md`](01-architecture.md) 3.4.1）。
> 色だけ `app.config.ts` で `primary: teal` / `neutral: slate` を指定し、それ以上は作り込まない。
> ダークモードはアプリバーのボタンで切り替える。既定はライト。OS の設定には追随しない。
> **設定画面にテーマ切替の項目は置かない**（3.11 に無いため）。切替はアプリバーに置く。
