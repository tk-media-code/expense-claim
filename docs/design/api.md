...
**集約するのは2本だけである。** どちらも `screens.md` 2.2 の**2手の経路上にある画面**で、
...
**逆に、会場とルートの画面は2手の経路に無い**（`screens.md` 2.1）。
**往復回数を減らす理由が無いので、資源単位に置く。**

```mermaid
flowchart LR
    subgraph path["2手の経路（screens.md 2.2）"]
        h["① ホーム"] --> r["② 交通費の記録"]
    end
...
**案件の詳細を集約しないのは、決定19 でここが経路から外れたからである**（`screens.md` 2.3）。
...
| **記録画面の既定値** | 「ルートが1本ならそれ、複数なら未選択」（`screens.md` 4.3）。**2手が成立するかどうかがこの規則にかかっている** |
| **提出行に書く金額**（往復の ×2、復路の反転） | `database.md` 5.2。**`expense_record_legs` は提出行の正本である。正本を作る規則を1か所に置く**（5.3） |
...
- **全画面を BFF にする**（`/api/screens/home` のように画面ごとに1本）— 往復は最小になるが、
  **画面が増えるたびに API が増える。** 設定系の画面は資源と1対1で、包む層が意味を持たない。
  しかも**導線は実際に動いた。** `screens.md` 2.3 はモックを触った結果、
...
**案件番号が空なら 422（`INVALID_VALUE`）。** 案件番号は必須である（F-10 / `screens.md` 3.4）。
...
**同じ理由で、絞り込みの検索パラメータも持たない**（F-38 / `screens.md` 3.6）。
...
| **金額に人の判断が入らない**（F-20 / `screens.md` 3.5） | **記録画面に入力欄が無い。** 2手の経路では、**金額を1円も送らない。**金額を直す入口は `PUT /api/segments/:id`（4.7）だけである |
...
**`database.md` 12章から引き継いだ2件は、[`error-handling.md`](error-handling.md) で閉じた。**
**どちらも「失敗したときにどう振る舞うか」であり、あの文書の主題そのものだった。**

| 何が | どう決まったか |
| --- | --- |
| **`attentions` の `detail` の書式** | **素のテキスト1本**（`error-handling.md` 4.1）。2.5 の `message` と同じ「本人が読む日本語」にそろえた。**画面から見て、要確認事項とエラーが同じ扱いになる** |
| **トランザクションの境界** | **外部呼び出しをまたいで開かない**（`error-handling.md` 6.1）。提出は**シートが先・`submissions` が後**で、間で落ちたら **`200` を返さない**（同 6.3）。シートは書けているので、**もう一度実行すれば揃う**（`F-29`） |

**`GET /api/home` の応答に `lastCronRunAt` が1つ増えている**（4.3）。
`error-handling.md` 7章が、**cron が落ちたことに気づく手立て**を
