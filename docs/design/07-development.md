...
| `frontend/app/layouts/default.vue` | アプリバー。各ページの `definePageMeta`（`title` / `back` / `menu`）から組む。ライト／ダークの切替もここ |
| `frontend/nuxt.config.ts` の `colorMode` | 既定は `light`。OS には追随しない |
