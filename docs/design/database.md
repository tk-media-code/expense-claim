...
**使っているルートが0本の区間は、出発駅・到着駅を直せる**（`api.md` 4.7 / `screens.md` 3.8）。
**重複の判定からは自分自身を除く。**
...
（[`error-handling.md`](error-handling.md) 7章）。
...
**この列もホームに出る**（`screens.md` 3.2）。`last_imported_at` と同じ形である —
...
以前は削除の操作そのものが無く（`api.md` 7章）、この2行は**起きない事態に備えた保険**だった。
いまは **`screens.md` 3.8 から消せる**ので、**この RESTRICT が「消せるか消せないか」を決めている。**
...
[`error-handling.md`](error-handling.md) で閉じた。**

| 何が | どう決まったか |
| --- | --- |
| **トランザクションの境界** | **外部呼び出しをまたいでトランザクションを開かない**（`error-handling.md` 6.1）。囲むのは DB の中だけで完結する不変条件で、**記録の保存なら `taxi_rides` と `receipts` の2本だけ**（同 6.2）。外部との整合は順番で守る |
| **`detail` の書式** | **素のテキスト1本。** 種別ごとの構造を持たせない（`error-handling.md` 4.1）。`screens.md` 3.10 が「種別・内容・発生日時」を並べるだけなので、**構造を持たせても展開する場所が無い。** `TEXT NOT NULL` は動かさなかった |

**あわせて 5.3 の `sync_state` に `last_cron_run_at` が1列増えている。**
[`error-handling.md`](error-handling.md) 7章が、**cron が落ちたことに気づく手立て**を
