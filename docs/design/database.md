...
**`submissions` / `sync_state` / `attentions` / `google_credentials` は、どこにも繋がない。**
...
> **サーバー側に状態が無いので、保存する列も無い。**
> 全端末を即座に失効させたくなった場合に何を足すかは、[`api.md`](api.md) で決める（12章）。
...
**投入するのは3行だけ。** `sync_state` と `google_credentials` の単一行、
そして `venues` は**空で始める**（F-13 でマスタを取り込む）。
...
| **全端末のセッション失効** | 署名付き Cookie なのでサーバー側に状態が無い（5.3）。即座に失効させたくなった場合に何を足すか | [`api.md`](api.md) |
