...
**画面にも API の応答にも出さない**（N-08 / `screens.md` 3.11 / `api.md` 7章）。
...
| 何が | どう決まったか |
| --- | --- |
| **トランザクションの境界** | **外部呼び出しをまたいで開かない**（`error-handling.md` 6.1）。6.1 の①〜③はトランザクションの外に置き、**囲むのは④の `taxi_rides` と `receipts` だけ**（同 6.2） |
...
（`error-handling.md` 6.2）。**孤児のファイルができるが、これを許す。**
