...
提出アラートは `docker compose` 内の cron コンテナが毎日起動して送るが（`architecture.md` 3.8）、
...
**打ちかけの運賃は残る。** **足す順は「駅 → 区間 → ルート」で固定である**（`api.md` 8章）。
**固定なのは順序であって、画面ではない。**
