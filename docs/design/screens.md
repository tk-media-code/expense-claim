...
**既存と重複する案件番号は弾く**（F-08 / [`api.md`](api.md) 4.4 の `PROJECT_NO_DUPLICATED`）。
...
**上書きにはキーボードが要り、置けば3手の経路（2.2）に入力欄が現れる。**
...
**金額を直せる場所は、この画面だけである**（F-20 / 3.5）。
記録画面にも 3.7 にも入力欄は無く、**片道運賃はここでしか変えられない。**
