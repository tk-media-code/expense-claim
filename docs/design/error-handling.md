...
- **扱わないこと** — 個々の外部呼び出しの叩き方（[`integration.md`](integration.md)）、
  エンドポイントの一覧（[`api.md`](api.md)）、画面の見た目（[`screens.md`](screens.md)）
...
| **`attentions.detail` の書式** | `database.md` 12章 / `api.md` 10章 / `integration.md` 11章 | **4章** |
| **`NF-10` 様式が変わったら止まる** | `architecture.md` 9章（**未確定事項ではなくトレーサビリティ表から**） | **5章** |
| **トランザクションの境界** | `database.md` 12章 / `api.md` 10章 / `integration.md` 11章 | **6章** |
| **定期実行が落ちたことに気づく手立て** | 要件定義13章 / `architecture.md` 11章 / `integration.md` 11章 | **7章** |
...
| 既に決まっていること | どこ |
| --- | --- |
| 失敗の分類（401 / 403・404 / ヘッダー相違 / 429 の振り分け） | `integration.md` 2.4 |
| **自動再試行を持たない**（指数バックオフの却下理由も） | `integration.md` 2.5 |
| エラー応答の形（**401 と 503 を分ける・502 と 503 を分ける**） | `api.md` 2.5 |
| **「止める」と「知らせて続ける」を応答の形で分ける** | `api.md` 2.6 |
| 中止する条件5つ・中止しない条件2つ | 要件定義 5.5 |
| 起きること × どう気づくか × どう直すか の12行 | 要件定義 10章 |
| `attentions` テーブルと `kind` の7種 | `database.md` 5.3 |
| 領収書保存の順番（保存 → 共有 → DB 書き込み） | `integration.md` 6.1 |
| 要確認事項の画面（**種別・内容・発生日時の平坦な表示**） | `screens.md` 3.10 |
| cron の置き場（compose 内の専用コンテナ・`Asia/Tokyo`） | `architecture.md` 3.8 |

**この表を引き取るということは、次の3つを前提に置くということである。**

| 前提 | 効き方 |
| --- | --- |
| **自動で再試行しない** | 失敗はその場で見せるか、要確認事項に残すかのどちらかになる。**「静かに再試行して静かに失敗する」経路を作らない**（`integration.md` 2.5） |
| **200 かどうかが「書いたか書いていないか」を表す** | 中止はエラー応答、続行は `200` に `warnings[]`（`api.md` 2.6）。**この約束を6章のトランザクション境界でも守る** |
| **要確認事項は「起きたことの記録」である** | 外から積むエンドポイントを持たない（`api.md` 4.9）。**アプリが自分で起こしたことだけが載る** |
...
**`attentions.kind` の7種**（`database.md` 5.3）が、**どの経路から生まれるか**を1か所にまとめる。
...
（`integration.md` 2.2「叩く呼び出しは9本だけ」）。
...
`warnings[]` で返すだけである（`api.md` 2.6）。

| なぜ | |
| --- | --- |
| **プレビューは書かない**（`api.md` 4.8） | **シートに未知のコードは載っていない。** まだ何も起きていない |
...
`api.md` 6.3 が「読めなければ要確認事項が残る」と決めているとおりである。
...
| **`occurred_at` を持っている**（`database.md` 5.3） | まとめると**いつ書いたかが消える** |
| **「同じとは何か」を決めずに済む** | 種別が同じなら同じか。会場コードまで見るか。**判定を持てば、その判定が壊れる** |

**確認済みにすれば消える**（`F-34`）。ホームが数えるのは未確認のものだけである
（`screens.md` 4.4）。**溜まって困るのは、確認していないときだけである。**
...
種別ごとの構造（JSON など）を持たせない。`database.md` 5.3 の `TEXT NOT NULL` は動かさない。

| なぜ | |
| --- | --- |
| **出す先が平坦である** | `screens.md` 3.10 は「種別・内容・発生日時」を並べるだけ。**構造を持たせても、展開する場所が無い** |
| **読むのは本人だけである** | 要確認事項を作るエンドポイントを持たず（`api.md` 4.9）、**機械が読み返す相手がいない** |
| **`api.md` 2.5 と同じ形になる** | エラー応答の `message` も「本人が読む日本語」で、**画面はそのまま出してよい**と決めてある。**画面から見て、要確認事項とエラーが同じ扱いになる** |
...
| `sheet_unreachable` | 何をしようとしたか（読み／書き）・**認可切れか共有停止か**（`integration.md` 2.4） | **スプレッドシートID・シート名** |
...
**認可切れなら再認可で直り、共有停止ならアプリ側では直せない**（`integration.md` 2.4）。
...
- **関連する行への外部キーを持つ**（`project_id` など）— 画面から案件へ飛べるが、
  **実績データは月度切替で消える**（`F-32`）。**参照先が消えた要確認事項が残る。**
  `attentions` が関連を張らないのは `database.md` 4章の決定であり、**そこを崩さない**
...
**`architecture.md` 9章が、この要件を丸ごとこの文書へ送っている。**
材料は揃っており、**組み立て直すだけである。**

| 何を | どこが決めている |
| --- | --- |
| 7行目のヘッダーを `values.get` で読む | `integration.md` 7.3 |
| 想定と違えば `502 SHEET_FORMAT_CHANGED` | `integration.md` 2.4 / `api.md` 2.5 |
| **中止する**（何も書かない） | 要件定義 5.5 |
| 中止はエラー応答で表す | `api.md` 2.6 |
...
**様式の変更だけが要確認事項に残らない。** `database.md` 5.3 の `kind` に
...
**要件定義 5.5 の「中止しない」2件は、`200` に `warnings[]` を載せる**（`api.md` 2.6）。
...
**ホームが「未記録」を出しており**（`screens.md` 3.2 / `F-21`）、
**同じことを2か所で知らせる意味が無い。**
...
| **cron も同じ DB に繋ぐ**（`architecture.md` 3.8） | 利用者1人でも接続は2系統ある。**長いトランザクションを常態にしない** |

**トランザクションで守るのは、DB の中だけで完結する不変条件である。**
外部との整合は、**順番で守る**（`integration.md` 6.1）。

### 6.2 記録の保存（`F-26`）

**`integration.md` 6.1 が順番を決めた。ここで決めるのは、どこを囲むかである。**
...
| 乗車の無い領収書を作らない | `receipts.taxi_ride_id` の外部キーと `UNIQUE`（`database.md` 5.2）が**そもそも作らせない** |

**③で失敗すると、ドライブに孤児のファイルが残る。** これを許す。

| なぜ | |
| --- | --- |
| **消す経路を持たないと決めてある** | `files.delete` を呼ばない（`integration.md` 6.3 / `api.md` 7章）。**消す実装を足せば「消してよいものを消す」判断がアプリに増える** |
...
502 で返す。`api.md` 2.6 の約束（`200` かどうかが「書いたか書いていないか」を表す）とは
**逆向きに見えるが、そうではない。**
...
**手順5の行挿入と手順8の書き込みは、別の呼び出しである**（`integration.md` 8.3）。
...
`api.md` 4.3 が「片方が失敗しても、もう片方は走る」と決めており、
**全体を1つのトランザクションにすると、その決定が崩れる。**

| 手順 | 囲むか |
| --- | --- |
| 1〜2 月度の検知と削除 | **`DELETE FROM projects` の1文だけ。** 子は `CASCADE` で落ちる（`database.md` 6.3） |
...
`imported_mails` の主キーは Gmail の message id で（`database.md` 5.3 / `F-08`）、
**取り込めた通は次回スキップされる。** まとめて囲むと、
**10通目で落ちたときに9通ぶんが消える。**

**`last_imported_at` は、1件も取り込まなかった実行でも更新する**（`api.md` 4.3）。
**Gmail の取り込みが失敗した実行では更新しない。**
この値はホームに出て、**静かな故障に気づくための手立てである**（要件定義10章 / `screens.md` 3.2）。
...
**`sync_state` には既に `last_alert_sent_on` がある**（`database.md` 5.3）。
...
1日空くのは時計やタイムゾーンのずれで起こりうるが（`Asia/Tokyo` 固定・`architecture.md` 3.8）、
**2日空けば落ちている。**
...
**「最後に取り込んだ日時」を置いたのと、まったく同じ形である**（`F-21` / `screens.md` 3.2）。
...
- **外部の死活監視サービスへ ping を送る**（`healthchecks.io` など）—
  ...
  却下する理由は2つ。**外部依存が1つ増え、目的②（無料構成への移行・`architecture.md` 1章）でも
...
| 文書 | 何が増えるか |
| --- | --- |
| [`database.md`](database.md) 5.3 | `sync_state` に **`last_cron_run_at`** を1列 |
| [`api.md`](api.md) 4.3 | `GET /api/home` の応答に `lastCronRunAt` |
| [`screens.md`](screens.md) 3.2 | ホームに「提出アラートが最後に動いた日時」 |

**新しいテーブルを作らず、`sync_state` に相乗りさせる。**

| なぜ | |
| --- | --- |
| **cron が書く列が、すでにそこにある** | `last_alert_sent_on` は cron が更新している（`database.md` 5.3）。**別のテーブルを立てると、cron の状態が2か所に割れる** |
| **読む先が同じである** | `last_imported_at` も `last_cron_run_at` も、**ホームが「静かな故障」を見せるために読む値**である（`screens.md` 3.2） |
...
**この文書で、設計の6本が揃った。**
`architecture.md` 11章・`database.md` 12章・`api.md` 10章・`integration.md` 11章が
ここへ送っていた宿題は、**すべて閉じた。**
