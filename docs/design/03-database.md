...
この文書は、要件定義 6章の論理データモデルを **MySQL 8.4 のテーブルへ落としたもの** である。
設計フェーズの3本目で、[`01-architecture.md`](01-architecture.md)・[`02-screens.md`](02-screens.md) の次に置く。

- **扱うこと** — 物理 ER 図、テーブル定義、型・NULL・既定値・索引・制約、削除の連鎖、
  マイグレーションの持ち方
- **扱わないこと** — エンドポイント（[`04-api.md`](04-api.md)）、外部 API の呼び出し設計
  （[`05-integration.md`](05-integration.md)）、異常系の実現方法（[`06-error-handling.md`](06-error-handling.md)）
- **出どころ** — 要件定義 5章（書き出し仕様）・6章（論理データモデル）、
  `01-architecture.md` 3.5 / 3.6 / 5.x / 7.2、`02-screens.md` 3.x（各画面が何を読むか）
...
### 2.2 `01-architecture.md` から来るもの

| 前提 | 出典 | 設計への効き方 |
| --- | --- | --- |
| **MySQL 8.4** | 3.5 | `CHECK` 制約・`utf8mb4_ja_0900_as_cs` が使える |
...
**MySQL 8.4 の既定 `utf8mb4_0900_ai_ci` を使わない。**
...
> **実機で確かめた**（12章 / Issue #75）。`stations` を流したときに濁点違いの2駅が両方入り、
> 同じ名前は重複で弾かれることも `backend/src/db/stations.integration.test.ts` が実テーブルの
> `UNIQUE (name)` に当てて確かめている（Issue #98）。
...
> **`repositories` に閉じる**（`01-architecture.md` 6.1 の縛り4）ため、
...
`02-screens.md` 3.7 も「別の駅として登録する」と画面に説明を出すとしている。
...
**この2つは登録のときだけでなく、更新のときにも当たる。**
**使っているルートが0本の区間は、出発駅・到着駅を直せる**（`04-api.md` 4.7 / `02-screens.md` 3.8）。
**重複の判定からは自分自身を除く。**

**区間は複数のルートで共有する**（要件定義 6.1 / R-23 / F-37）。
最寄り駅が同じ会場が複数あるとき、「自宅最寄り駅→乗換駅」は多くのルートに現れる。
**ルートごとに運賃を持つと、運賃改定で直し漏れが起き、同じ区間なのに違う額が提出される。**
記録画面はルートの登録運賃をそのまま既定値にするので（`02-screens.md` 3.5）、
**壊れ方が「静かに違う額が出る」になる。**
...
**区間は「自宅→会場」の向きで持つ片道のデータである**（決定9 / 決定28）。
`from_station_id` は自宅側、`to_station_id` は会場側の駅を指す。
ルートが「自宅→会場」の1方向なので、**区間もその向きでしか登録されない。**
復路は同じ区間を逆順にし、出発駅と到着駅を入れ替えて使う。
**運賃はその区間の `one_way_fare` をそのまま使う。**

**だから逆向きの区間は登録できない。** 同じ2駅の逆向きは、同じ区間を復路に使ったものであって別の区間ではない。
登録できると同じ区間に運賃が2つある状態になるので、`UNIQUE` を張った理由がそのまま当てはまる。
**`UNIQUE (from_station_id, to_station_id)` は方向付きのままで、逆向きを含めた一意性は `services` が守る**
（登録・駅の差し替えで両方向を引く）。利用者は1人で、同時に2つの登録は起きない。
列名を `home_side` / `venue_side` のように向きを語る名前へ改めることはしない（決定28）。
...
- **同じ駅ペアに複数の運賃を許す**（`UNIQUE` を張らない） — どれが正か決まらない。
  **実額とずれたら区間の運賃を直せばよい**（F-37）。設定データを2つに割る理由にならない
...
- **`routes.segment_ids` に配列（JSON）で持つ** — **外部キーが張れない。**
  ...
  あわせて「この区間を何本のルートが使っているか」（`02-screens.md` 3.8）が JSON 検索になり、
  **MySQL 固有の機能に寄りかからない**（`01-architecture.md` 6.1 の縛り4）に反する。
...
**寿命は「切り替わった先より前の月度のものが、提出の有無を問わず消える」**（F-32 / 6章）。
...
（`02-screens.md` 3.3 の記録の要約）。**提出行は `expense_record_legs` から作る**（7章）。
...
| 提出シートの姿 | どこで作るか |
| --- | --- |
| **往復の合計額**（片道運賃 ×2） | **記録時**（`02-screens.md` 3.5「登録運賃、`往復` なら ×2」） |
...
| `last_imported_at` | `DATETIME` | YES | NULL | **最後に取り込んだ日時**（`02-screens.md` 3.2） |
| `last_seen_target_month` | `DATE` | YES | NULL | 最後に読んだ提出シートの月度。`CHECK` で月初のみ |
| `last_alert_sent_on` | `DATE` | YES | NULL | **最後に提出アラートを送った日**（下記） |
| `last_cron_run_at` | `DATETIME` | YES | NULL | **cron が最後に起動した日時**（下記） |
...
アラートは `docker compose` 内の専用コンテナが日1回起動して送る（`01-architecture.md` 3.8）が、
**再デプロイやコンテナの再起動で、同じ日にもう一度起動しうる。**
F-29 が提出について言う「何度でも実行できて、結果が同じ」を、**アラートにも当てる。**

**`last_imported_at` はホームに出る**（`02-screens.md` 3.2）。
**差出人アドレスが変わると、エラーは何も出ずに案件が入ってこなくなる**（要件定義 10章）。
気づく手立てがこの1個しかないので、**取り込みが1件も無かった実行でも更新する。**

**`last_cron_run_at` は、cron が生きているかを見るために持つ**
（[`06-error-handling.md`](06-error-handling.md) 7章）。
**cron は起動するたびにこの列を更新する。** 1日でも3日でもなく、何もせずに終わる日も更新する。

**`last_alert_sent_on` では足りない。** あちらは「最後に**送った**日」で、月2回しか動かない。
**送っていない日が続いているとき、走っていて送るものが無いのか、走っていないのかが
区別できない。** 死活を見るには、**送信と無関係に毎日動く値**が要る。

**この列もホームに出る**（`02-screens.md` 3.2）。`last_imported_at` と同じ形である —
**落ちたことを知らせる主体がいないので、読む側が気づくしかない。**
...
- `INDEX (checked_at, occurred_at)` — ホームの件数は「未確認のもの」だけを数える（`02-screens.md` 3.2）
...
**未確認だけを数えれば、ホームの見え方は同じである**（`02-screens.md` 4.4）。
...
| --- | --- | --- | --- | --- |
| `id` | `TINYINT UNSIGNED` | NO | 1 | 主キー。`CHECK (id = 1)` |
| `refresh_token_encrypted` | `VARBINARY(1024)` | NO | — | **暗号化して保存する**（`01-architecture.md` 7.2） |
...
**暗号鍵は環境変数で持つ**（`01-architecture.md` 7.3）。**列に鍵は入れない。**

**`scopes` を持つのは、スコープが増えたときに気づくためである。**
`gmail.send` は後から足したものであり（`google-cloud-basics.md` 8節）、
**古いトークンのままだとアラートの送信だけが失敗する。** 失敗してから気づくより、
**保持しているスコープを見て再認可へ導けるほうがよい**（F-02 / `02-screens.md` 3.11）。

> **端末ごとのセッションのテーブルは持たない。**
> セッションは署名付き Cookie で、有効期限は90日・使うたび延びる（`01-architecture.md` 3.7）。
...
`POST /api/auth/logout-all` がここに現在時刻を書き（[`04-api.md`](04-api.md) 4.1）、
...
（`01-architecture.md` 3.7 が2つを分けている）。**同じ行に置くと、その区別が消える。**
...
| **実績データ**（`projects` とその子） | **月度の切り替わりを検知したとき。** 消えるのは**切り替わった先より前の月度のもの**で、**提出の有無は問わない**（決定27） | F-32 / 要件定義 6.4 |
| **`submissions`** | **消さない**（下記） | 要件定義 6.4 |
| **`imported_mails`** | **消さない。** 再取り込みを防ぐため | 要件定義 6.3 / 6.4 |
| **`attentions`** | 確認済みにしたら消してよいが、**消さない**（5.3） | 要件定義 6.4 |
| **`auth_state`** | **消さない。** 単一行で、`sessions_valid_after` が上書きされるだけ | [`04-api.md`](04-api.md) 4.1 |
...
**そして残すほうが正しい。** 提出済みかどうかと最後に提出した日時は、ホーム（F-30）と
提出アラート（F-36。提出が済んでいれば送らない）が読む。**切替で消す理由が無い。**
...
| **`venues` → `routes`** | **RESTRICT** | 設定データを巻き込みで消さない。**会場の削除は画面に無い**（`02-screens.md` 3.6） |
| **`segments` → `route_segments`** | **RESTRICT** | **使われている区間を消せない。** 他のルートがまだ使っている。**画面から見える制約になった**（下記） |
| **`stations` → `segments`** | **RESTRICT** | 使われている駅を消せない。**消せてしまうとルートが壊れる。画面から見える制約になった**（下記） |
...
**区間と駅の RESTRICT は、画面から見える制約である。**
`02-screens.md` 3.8 から消せるので、この RESTRICT が「消せるか消せないか」を決めている（決定22）。
API は 409 として返し、画面は削除ボタンを出さずに理由を書く。

6.1 は設定データを「消えない。本人が消したときだけ」と書いている。
RESTRICT が拒むのは使われているものであって、巻き込みで消える経路は1本も生まれない。

**ルートを消しても、区間は消えない。**
最寄り駅が同じ会場が複数あるとき、「自宅最寄り駅→乗換駅」は多くのルートに現れる。
`segments` を独立させているので、運賃は1か所にあり、ルートの削除はその並びしか落とさない（決定18）。

### 6.3 月度切替の削除をどう引くか

**F-32 の条件は1つである。切り替わった先の月度より前の案件を消す。** 提出の有無は見ない（決定27）。

**月度の列を持たないので、施行日の範囲で引く**（9章）。

```sql
-- :next_month は今読んだ提出シートの A1（＝切り替わった先）
DELETE FROM projects WHERE service_date < :next_month;
```

**`expense_records` / `expense_record_legs` / `taxi_rides` / `receipts` は CASCADE で落ちる。**

**切替先以降の月度（＝これから稼働する案件）は残る。** まだ提出時期が来ていないだけで、消してよいものではない
（要件定義 4.8）。切替期間（翌月4〜7日）はその案件の稼働日と重なる。

**未提出のまま切り替わった月度も消える。** 残しても本人にできることは「手で入力する」で変わらず（決定14）、
ホームの範囲（対象月度以降）からも外れて、どこからも見えないデータになる。要確認事項にも積まない。
...
- **ルートを削除させない（論理削除にする）** — 参照を保てるが、
  **N-06 が言う「残り続ける設定データ」に、本人が消したつもりのものが混ざる。**
  会場とルートの画面（`02-screens.md` 3.6）が、消したはずのルートで埋まっていく
...
画面に会場名を出せない**（`02-screens.md` 3.2 の案件カード）。
...
**施行日は画面から直せる**（F-11 / `02-screens.md` 3.3「直すと所属月度が変わる」）。
...
│  │  └ client.ts        接続
│  └ repositories/       Drizzle を触るのはここだけ（01-architecture.md 5.2）
...
| **`repositories` の外へ Drizzle の型を出さない** | `01-architecture.md` 5.2。DB を替えたときの影響を1層に閉じる |
| **`domain` の型へ詰め替える** | 「月」の3基準を型で分ける（`01-architecture.md` 5.4）。**テーブルの行そのままでは区別が付かない** |
...
`google_credentials` は投入できないので、初期データは2行である。

**`stations` / `segments` / `routes` / `route_segments` も空で始まる。**
本人が登録する（F-15 / F-16 / F-37）。
**設定データの控えから読み込むこともできる**（NF-11 / `01-architecture.md` 8章）。
...
（`01-architecture.md` 3.7 で決めた）。**実現方法から生まれたテーブルなので、論理モデルに対応が無い。**
...
| `F-20` 金額は記録時点で確定し、後から動かない | `expense_record_legs.amount` |
...
| `N-20` 月度は施行日で判定する | 9章 |
| `NF-11` 設定データの控え | 10.3 / `01-architecture.md` 8章 |
| `NF-13` 送るメールの宛先は本人固定 | **テーブルを持たない。** 環境変数から取る（`01-architecture.md` 2.1） |

## 12. 未確定事項

| 何が | 何が決まっていないか | いつ決めるか |
| --- | --- | --- |
| **接続数と接続の持ち方** | 利用者1人なので小さいが、**cron のコンテナも DB に繋ぐ**（`01-architecture.md` 3.8） | 実装時 |

**要件としては固まっている。** ここに挙げたのは実現方法の選択である。

> **照合順序の挙動は実機で確かめて閉じた**（2026-09-09 / Issue #75）。
> `backend/src/db/collation.integration.test.ts` が、テスト用スキーマの既定が
> `utf8mb4_ja_0900_as_cs` であること、その照合順序で `か` と `が` が別の値になること、
> そして **MySQL 8.4 の既定 `utf8mb4_0900_ai_ci` なら同じ値になってしまうこと**を、
> 実 MySQL に当てて確かめている。4.1 の懸念はそのとおりだった。
閉じた宿題（セッション失効、トランザクション境界、`detail` の書式、cron の死活）は
[`docs/decisions.md`](../decisions.md) と決まった先の文書を見る。
