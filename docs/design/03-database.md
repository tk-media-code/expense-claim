...
**区間と駅の RESTRICT は、画面から見える制約になった。**
以前は削除の操作そのものが無く（`04-api.md` 7章）、この2行は**起きない事態に備えた保険**だった。
いまは **`02-screens.md` 3.8 から消せる**ので、**この RESTRICT が「消せるか消せないか」を決めている。**
API は 409 として返し、画面は削除ボタンを出さずに理由を書く。

**守る範囲は変わっていない。** 6.1 は設定データを「消えない。**本人が消したときだけ**」と
書いており、**消える道はもとから想定されていた。**
RESTRICT が拒むのは**使われているもの**であって、**巻き込みで消える経路は1本も生まれない。**
...
[`04-api.md`](04-api.md) 4.1 が **`auth_state` に失効の基準時刻を1つ持つ**と決め、
その結果として 5.3 にテーブルが1本増えている。

**「トランザクションの境界」と「`detail` の書式」は、
[`06-error-handling.md`](06-error-handling.md) で閉じた。**

| 何が | どう決まったか |
| --- | --- |
| **トランザクションの境界** | **外部呼び出しをまたいでトランザクションを開かない**（`06-error-handling.md` 6.1）。囲むのは DB の中だけで完結する不変条件で、**記録の保存なら `taxi_rides` と `receipts` の2本だけ**（同 6.2）。外部との整合は順番で守る |
| **`detail` の書式** | **素のテキスト1本。** 種別ごとの構造を持たせない（`06-error-handling.md` 4.1）。`02-screens.md` 3.10 が「種別・内容・発生日時」を並べるだけなので、**構造を持たせても展開する場所が無い。** `TEXT NOT NULL` は動かさなかった |

**あわせて 5.3 の `sync_state` に `last_cron_run_at` が1列増えている。**
[`06-error-handling.md`](06-error-handling.md) 7章が、**cron が落ちたことに気づく手立て**を
「走った時刻を残し、ホームに出す」と決めた結果である。
**テーブルは増えていない** — cron が書く列（`last_alert_sent_on`）が、すでにここにあった。
