...
- **扱わないこと** — 画面の見た目（[`02-screens.md`](02-screens.md)）、テーブル定義（[`03-database.md`](03-database.md)）、
  エンドポイント（[`04-api.md`](04-api.md)）、外部 API の呼び出し設計（[`05-integration.md`](05-integration.md)）
...
[`docs/decisions.md`](../decisions.md) へ移す。要件そのものが変わった場合は、要件定義も直す。
...
| **Workers の CPU 時間は無料プランで 1リクエスト 10 ms。** DB や fetch の待ち時間は数えない | 重い処理をリクエストの中でやらない。提出処理が収まるかは未検証（10章） |
| **Hyperdrive が無料プランで使え、PostgreSQL と MySQL に対応する。ただし DB 本体は持たない。** 別に用意した DB への接続プールとキャッシュである | DB エンジンの選択は移行の妨げにならない。**DB をどこで無料で動かすかは別問題**で、目的②で決める |
| Workers に無料枠がある（1日10万リクエスト。静的アセットへのリクエストは数えない） | 利用者1人には過剰なほど余る |
| Worker のサイズ上限は非圧縮 64 MiB（全プラン共通） | 選定には効かない |

**この制約は今回の成果物には課されない。** EC2 で動かすからである。
だが、**ここで Workers で動かないものを選ぶと目的②で作り直しになる**ので、選定の判断材料として使う。

> **当初は Workers の圧縮後のバンドル上限を、重いフレームワークを選べない制約として置いていた。**
> この上限は 2026-09-04 に撤廃された。選定は変えていない。経緯は [`docs/decisions.md`](../decisions.md) の閉じた宿題にある。
...
- **Python** — 学習資料は最も多く、Workers でも動く（`python_workers` フラグ）。だがフロントと言語が分かれる
...
- **Express** — Node.js 前提。Workers では `nodejs_compat` を有効にして動かす後追いの形になる
...
利用者1人・11画面・認証必須で、SEO も初回表示速度も要求されていないため、
**サーバー側で描画する理由が無い。** バックエンドとの境界も明確になる（4.3）。

**採らなかった案**

- **Next.js** — React なので今回は選べない。加えて **Cloudflare 対応はアダプタ経由の後追いである。**
  Workers で動かすには OpenNext というアダプタを噛ませ、ISR などのキャッシュを使うなら R2 などの
  バインディングも足すことになり、構成部品が増える。**Next.js は Vercel のために作られている**という
  構造的な差がある
...
### 3.4.1 UI 基盤 — Nuxt UI 4

**見た目はモックに合わせない**（2026-09-09 本人確認）。
[`mockup/index.html`](mockup/index.html) は **UI の機能と導線の正本**であって、
**配色・タイポグラフィの正本ではない。** モックが持つデザイントークンとダークモードの
作り込みは移植せず、**Nuxt UI の既定テーマに委ねる。**

| なぜ | |
| --- | --- |
| **Tailwind v4 を内包する** | 別途入れる必要がなく、ユーティリティもそのまま書ける |
| **Nuxt 公式なので統合が薄い** | モジュール1行で auto-import まで通る。ラッパーモジュールを挟まない |
| **振る舞いとアクセシビリティが入っている** | Reka UI ベース。モーダル・タブ・トグルのキーボード操作とスクリーンリーダ対応を自分で書かずに済む |
| Zod でのフォーム検証が統合済み | `UForm` がそのまま受ける |

**採らなかった案**

- **Tailwind 単体** — ユーティリティだけで、ボタンもモーダルも自分で設計することになる。
  **「フレームワークを入れれば整った見た目になる」という要求を満たさない**
- **Vuetify 4** — Material Design 3 そのもので日本語資料も多いが、**独自のスタイルシステムなので
  Tailwind と併存しない。** 選ぶなら Tailwind の決定を撤回することになる
- **PrimeVue 5** — unstyled モードで Tailwind と併用できるが、**11画面・利用者1人に対して過剰**
- **daisyUI** — 最も軽いが **JS の振る舞いを持たない。** モーダル・タブ・フォーカス管理を自分で書く
- **Chakra UI / Material UI（MUI）** — **どちらも React 専用で、そもそも Vue に載らない。**
  Vue 版 Chakra（`@chakra-ui/vue-next`）は `1.0.0-alpha.16` で1年以上停止している。
  加えて前回のアプリが React SPA なので、**課題の縛り（スタックを変える）からも二重に外れる**

**UI ライブラリの重さは目的②には関係が無い。**
SPA は静的ファイルとして配られ、**Worker のコードには含まれない**（6.2）。
重さが効くのは初回表示の速さであり、利用者1人・認証必須のこのアプリでは効かない。
...
| **移行の妨げにならない** | Cloudflare Hyperdrive が **無料プランで MySQL に対応**している。**MySQL 本体の置き場所は別に要る**（2.3 / 6.2） |
...
| **基本のクエリの書き方が DB で変わらない** | select・where・join・update・delete は MySQL・PostgreSQL・SQLite で同じ書き方になる。方言に固有の呼び出しは DB を替えるときに書き直す（6.2） |
| SQL に近い | 生成される SQL が読める。**提出処理のような込み入った操作で何が起きているか分かる** |
| マイグレーションが付く | `drizzle-kit` でスキーマの変更履歴を持てる |

**採らなかった案**

- **Prisma** — スキーマファイルが宣言的で、そのまま [`03-database.md`](03-database.md) の材料にできるのが魅力。
  **だが Workers で使うにはドライバアダプタを挟む必要があり、Drizzle より部品が増える。**
  生成される SQL も見えにくく、提出処理のような込み入った操作で何が起きているか追いにくい
...
**屋外・移動中に再ログインを求めるのは、「2手で終わる」（要件定義 8.1）が最も崩れる形である。**
...
| RDS | 1台。MySQL 8.4 |
| Terraform | 前回のものを引き継ぎ、このリポジトリの [`infra/`](../../infra/README.md) に置く |

**動作確認のために立てる環境は使い捨てである。** HTTP + パブリック IP で、HTTPS もドメインも持たない。
Google のウェブアプリ型 OAuth はリダイレクト URI に https とドメイン名を要求するため、
AWS 上では `GOOGLE_STUB=1` で動かし、確認が済んだら `terraform destroy` する
（[決定26](../decisions.md#決定26--aws-の確認環境)）。4.1 の構成図の HTTPS は、ドメインを持つ運用に移ったときの姿である。
...
（[`03-database.md`](03-database.md) 4.2）。

**中身は crond ではなく、毎朝 07:00（JST）まで眠って起きる1つの Node.js プロセスである**
（`backend/src/scheduler.ts`・2026-09-15）。コンテナは `node` ユーザーで動き（`07-development.md` 2.5）、
busybox の crond は root を要る。1日1回の判定と送信だけなので、時刻まで眠るプロセスで足りる。
起動時にも1回走る。同じ日に2通送らないことは `sync_state.last_alert_sent_on` が守る。
...
**目的②でも同じ形が取れる。** Cloudflare では1つの Worker に静的ファイルを同梱でき、
`/api/*` だけを Worker のコードで受け、それ以外を静的ファイルとして返せる
（Workers Static Assets の `run_worker_first`）。**移行しても CORS は発生しない。**
...
| --- | --- | --- |
| フロントエンド | EC2 の nginx が配る静的ファイル | **Workers Static Assets**（Worker に同梱する静的ファイル。リクエストは無料で無制限。Cloudflare は新規プロジェクトには Pages ではなくこちらを推奨している） |
| バックエンド | EC2 の Node.js コンテナ | **Cloudflare Workers**（無料枠 1日10万リクエスト。**CPU 時間 1リクエスト 10 ms**） |
| データベース | RDS MySQL | **別に用意する DB。** Hyperdrive は接続プールとキャッシュで、DB 本体を持たない。**DB をどこで無料で動かすかは目的②で決める**（候補は Cloudflare の D1 か、Hyperdrive 経由で繋ぐ外部の無料 MySQL） |
| 秘密の置き場 | 環境変数 | Workers Secrets |
| ルーティング | nginx がパスで振り分け | 1つの Worker が `/api/*` を受け、それ以外を静的ファイルとして返す（4.3） |

**Hono・Nuxt・Drizzle は、いずれもそのまま動く。** ただし DB に D1 を採ると SQLite になるので、
Drizzle のスキーマ定義と `repositories` は書き直しになる。**D1 は対話的なトランザクションを持たず `batch()` だけ**なので、
`db.transaction()` の箇所は組み替えが要る。`$returningId()` などの MySQL 専用の呼び出しと mysql2 の
エラーコードの判定も、SQLite 向けに書き直す（6.1 の縛り4で、影響はその層に留まる）。

### 6.3 移行するとき書き直しが要るもの

**パッケージとしては `googleapis` だけである。** DB に D1 を採るなら、6.2 のスキーマ定義と `repositories` も加わる。

`googleapis` は Node.js への依存が重く、
**Workers ランタイムで動かない可能性が高い**（未検証・10章）。
...
> [`05-integration.md`](05-integration.md) 8.2 が **クリアという操作そのものをやめた**ため、1本減った。
...
**いずれもリポジトリに入れない**（`NF-07`）。雛形は [`.env.example`](../../.env.example)。
アプリ本体の変数と、検証ツール（`tools/*-probe`）専用の変数を分けて書いてある。実値は入れない。
...
| `NF-09` 行数・保護範囲を決め打ちにしない | [`05-integration.md`](05-integration.md) 7.2（`protectedRanges` の `requestingUserCanEdit` から書ける範囲を決める） |
| `NF-13` 送るメールの宛先は本人固定 | 2.1 / 7.3（宛先は環境変数。実行時に決めない） |
| `NF-10` 様式が変わったら止まる | [`06-error-handling.md`](06-error-handling.md) 5.1（**中止条件が、すべて書き込みより前に並んでいる**） |
| `NF-11` 設定データの控え | 8章 |
...
| 前提 | 出どころ | いつ確かめるか |
| --- | --- | --- |
| **`googleapis` が Cloudflare Workers で動かない** | Node.js 依存からの推測 | **目的②のプロジェクト。** 今回は 5.3 で備えるだけ |
| **Hyperdrive 経由で、別に用意した MySQL に Workers から繋がる** | Cloudflare の公式ドキュメント（MySQL 対応は 2026-08-07 に GA。Hyperdrive は無料プランに含まれる） | 目的②のプロジェクト |
| **提出処理が Workers の CPU 時間制限（無料プランで 1リクエスト 10 ms）に収まる** | 提出行が月20〜30行と小さいことからの推測 | 目的②のプロジェクト |
