...
[`03-database.md`](03-database.md) で閉じた。** 命名規則は `05-integration.md` で決めるとしていたが、
**保存する列（`receipts.file_name`）と同じ場所で決めるほうが、根拠と結果が離れない。**

**「エンドポイントの粒度」と「全端末のセッション失効」は、[`04-api.md`](04-api.md) で閉じた。**

| 何が | どう決まったか |
| --- | --- |
| エンドポイントの粒度 | **リソース中心に、集約を2本だけ混ぜる**（`04-api.md` 3章）。集約するのはホームと交通費の記録で、**どちらも「2手」の経路上にあり、開いた時点で揃っていることが要件そのもの**だから |
| 全端末のセッション失効 | **単一行のテーブルに、失効の基準時刻を1つだけ持つ**（`04-api.md` 4.1 / [`03-database.md`](03-database.md) 5.3 の `auth_state`）。**端末ごとの行を持たないので掃除が要らない。** 3.7 の「有効期限90日」は動かさず、切る手立てだけを足した |

**「定期実行が落ちたことに気づく手立て」と `NF-10` は、
[`06-error-handling.md`](06-error-handling.md) で閉じた。**

| 何が | どう決まったか |
| --- | --- |
| **定期実行が落ちたことに気づく手立て** | **cron は起動するたびに時刻を残し、それをホームに出す**（`06-error-handling.md` 7.2 / [`03-database.md`](03-database.md) 5.3 の `sync_state.last_cron_run_at`）。**要確認事項には積まない** — 落ちている cron に自分の不在は記録できず、**「無いこと」は記録できない。** あわせてコンテナに `restart: unless-stopped` を入れる（**回復であって検知ではない。両方が要る**） |
| **`NF-10` 様式が変わったら止まる** | **要件定義 5.2 の手順の並びが、そのまま実装になっている**（`06-error-handling.md` 5.1）。中止条件は5つとも書き込みより前にあり、**7行目のヘッダーを読むのは手順3である** |

**3.8 の採らなかった案（ホストの cron / systemd タイマー）に、もう1つ足りていなかった。**
**「落ちたことをどう知るか」は、置き場所を変えても解けない。**
外部の死活監視サービスへ ping を送る案も検討したが、
**「監視サービスが落ちたことを誰が知るのか」という同じ問いが1段ずれるだけである**
（`06-error-handling.md` 7.4）。
