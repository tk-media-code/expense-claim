...
**開発者自身が、このアプリを使うビデオグラファーである。**
交通費は、その月の分を翌月3日の正午までに委託元のスプレッドシートへ書く。いつ書いてもよいのだが、
**入力が面倒で撮影の日には書かず、ずるずる持ち越して、3日の直前に気づくことが多かった。**

会場ごとに決まった形があれば入力も楽なのだが、**毎回同じにはならない。**
経路は同行するメンバーの都合で変わるし、JR だけで帰れるはずが、JR が大幅に遅延していて、
別の路線で帰ることもある。タクシーも、乗ったからといって毎回自分が払うわけではない。
同行者が代わりに払ってくれて自分の経費にならない日もあれば、自分が払う日もあり、
それはその場の雰囲気で決まる。**経路もタクシー代も、その日にならないと決まらないので、
持ち越したあとで思い出すのがしんどい。**

| 困っていたこと | このアプリでは |
| --- | --- |
| 入力が面倒で、撮影の日に書かない | 帰り道にスマートフォンから、**案件と使ったルートを選ぶだけ**で記録が終わる。案件は依頼メールから自動で入っている |
| 経路もタクシー代も毎回違い、使い回しがきかない | 会場ごとに**複数のルートを登録**しておき、その日に使ったものを選ぶ。往路と復路が違ってもよく、タクシー代は自分が払った日だけ足す |
| 持ち越したあとで、経路を思い出して書く | その日のうちに記録が済むので、**思い出す作業そのものが無くなる** |
| 3日の直前まで気づかない | 1日と3日の朝、**未提出ならメールで知らせる** |

提出は、内容を確認して実行するだけになる。乗り換えごとに行を分けるのも、
タクシーの領収書をドライブへ保存して URL を書くのも、アプリがやる。
