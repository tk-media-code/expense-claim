...
月度ごとに提出する。使うのは本人1名だけで、スマートフォンのブラウザから使うことを主に想定している。
案件は委託元からの依頼メールを取り込んで作られ、提出漏れは毎朝の定期実行がメールで催促する。

> **このアプリは Google の認証（ログイン）と認可（Gmail・ドライブ・スプレッドシートへのアクセス）を
> 実装している。ただし、AWS 上に立てる確認環境では実際の Google の認証・認可は通らない。**
> Google のウェブアプリ型 OAuth クライアントは、リダイレクト URI に https とドメイン名を要求する。
> この環境は HTTP とパブリック IP しか持たないため、リダイレクト URI を登録できない。
> そのため AWS 上では、Google を叩かない実装に差し替えて動かしている。
> **実物の Google に繋いだ認証・認可は、Cloudflare で動かすアプリ側で対応している。**
...
> 氏名・メールアドレスは書かない。この README に出る例はすべて架空の値である。
...
- **オリジンを分けない。** `/` は SPA、`/api/*` は Hono へ nginx が振り分ける。
  **CORS と Cookie の面倒を利用者1人のアプリで抱えない**
...
- **定期実行は1本しか置かない。** ジョブキューもワーカーも無い。crond でもなく、**時刻まで眠る Node.js プロセス**である
...
- **HTTPS もドメインも無いため、Google のウェブアプリ型 OAuth が通らない。** そのため AWS 上では
  Google を叩かない実装で動かし、確認が済んだら `destroy` する**使い捨ての環境**である
...
- **要求分析**（なぜ作るのか・何が欲しいのか）: [`docs/requirements-analysis.md`](docs/requirements-analysis.md) —
  ヒアリングと、提出シート・依頼メールの実測から整理した
- **要件定義**（いま守る約束）: [`docs/requirements.md`](docs/requirements.md) —
  機能要件と非機能要件、そして要求とのトレーサビリティ
